...
A sample compose file for an entire setup of both components and a database can be found in [sample.compose.yml](https://github.com/swiyu-admin-ch/eidch-verifier-agent-management/blob/main/sample.compose.yml) file. **Replace all placeholder in angle brackets `<VARIABLE_NAME>`**. In addition to that you need to adapt the
